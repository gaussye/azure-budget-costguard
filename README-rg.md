...
| `[alert-email]`       | 否   | 接收两级预算通知的邮箱;支持**多个邮箱**,用逗号分隔(建议整体加引号) | `you@example.com`  |
...
> 脚本会自动收集第 4 个及之后的所有参数,**无论从 cmd.exe 还是 PowerShell 运行都能正确解析**
> (从 PowerShell 运行时引号会被剥掉、逗号被拆成多个参数,脚本已做兼容)。建议仍加引号以防路径/空格问题。
